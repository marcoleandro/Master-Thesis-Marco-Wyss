...
impact_factor <- read.csv("/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/impact factor data/scimagojr 2020_clean.csv", sep = ",", header = T)

abs <- readxl::read_xlsx("/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/Abstracts/round 3/sample_SM_train_new_search_string_12.xlsx")
sum(duplicated(abs$Article.Title)) # 48 duplicates
abs <- abs[!duplicated(abs$Article.Title),]
sum(is.na(abs$Abstract))
abs <- abs[!duplicated(abs$Abstract),]

nrow(abs[!is.na(abs$feasibility) & !is.na(abs$Abstract),])/11580

abs$feasibility <- ifelse(abs$junk == 1, 0, abs$feasibility)
abs$policy_evaluation <- ifelse(abs$junk == 1, 0, abs$policy_evaluation)
abs$climate <- ifelse(abs$junk == 1, 0, abs$climate)
abs$environment <- ifelse(abs$junk == 1, 0, abs$environment)
abs$mitigation <- ifelse(abs$junk == 1, 0, abs$mitigation)
abs$adaptation <- ifelse(abs$junk == 1, 0, abs$adaptation)
abs$energy <- ifelse(abs$junk == 1, 0, abs$energy)
abs$transport <- ifelse(abs$junk == 1, 0, abs$transport)
abs$energy <- ifelse(abs$junk == 1, 0, abs$energy)
abs$buildings <- ifelse(abs$junk == 1, 0, abs$buildings)
abs$industry <- ifelse(abs$junk == 1, 0, abs$industry)
abs$afolu <- ifelse(abs$junk == 1, 0, abs$afolu)
abs$economic_cost <- ifelse(abs$junk == 1, 0, abs$economic_cost)
abs$distributional_dynamics <- ifelse(abs$junk == 1, 0, abs$distributional_dynamics)
abs$institutional_capacity <- ifelse(abs$junk == 1, 0, abs$institutional_capacity)
abs$multilevel <- ifelse(abs$junk == 1, 0, abs$multilevel)

variables <- c("feasibility", "policy_evaluation", "climate", "environment", "mitigation", "adaptation", # topics
               "energy", "transport", "buildings", "industry", 
               # "waste", 
               "afolu", # sectors
               "economic_cost", "distributional_dynamics", "institutional_capacity", "multilevel") # barriers
...
my_rec <- function(outcome) {
  abs_train %>% 
    select(Abstract, Article.Title, Author.Keywords, Keywords.Plus, {{outcome}}) %>% 
    recipe() %>% 
    update_role(Abstract, Article.Title, Author.Keywords, Keywords.Plus, new_role = "predictor") %>%
    update_role({{outcome}}, new_role = "outcome") %>%
    step_tokenize(Abstract, Article.Title, Author.Keywords, Keywords.Plus, token = "word_stems") %>%
    ...
    step_stopwords(Abstract, Article.Title, Author.Keywords, Keywords.Plus) %>%
    step_tokenfilter(Abstract, Article.Title, Author.Keywords, Keywords.Plus, max_tokens = 1e3) %>%
    step_tfidf(Abstract, Article.Title, Author.Keywords, Keywords.Plus)  %>%
    step_smote({{outcome}}) 
}
...
var_names <- c("Feasibility", "Policy Evaluation", "Climate", "Environment", "Mitigation", "Adaptation", "Energy", "Transport", "Buildings", "Industry",
              "AFOLU", "Economic Cost", "Distributional Dynamics", "Institutional Capacity", "Multi-Level Governance")
topics <- c("Feasibility", "Policy Evaluation", "Climate", "Environment", "Mitigation", "Adaptation")
sectors <- c("Energy", "Transport", "Buildings", "Industry", "AFOLU")
barriers <- c("Economic Cost", "Distributional Dynamics", "Institutional Capacity", "Multi-Level Governance")
...
evaluate_model_performance <- function(res_boost, model_boost){
  ...
  performance$feature <- factor(c(rep("Barriers", each = length(unique(performance$.metric))*length(barriers)), 
                                  rep("Sectors", each = length(unique(performance$.metric))*length(sectors)),
                                  rep("Topics", each = length(unique(performance$.metric))*length(topics))))
  performance
}
...
ggsave(p_boost, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/fold_and_oos_xgb.pdf", height = 6, width = 13)
ggsave(p_svm, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/fold_and_oos_svm.pdf", height = 6, width = 13)
ggsave(p_forest, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/fold_and_oos_forest.pdf", height = 6, width = 13)
ggsave(p_lasso, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/fold_and_oos_lasso.pdf", height = 6, width = 13)
...
# save the plots
ggsave(share_boost, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/Share_XGBoost.pdf", height = 5, width = 10)
ggsave(share_forest, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/Share_Forest.pdf", height = 5, width = 10)
ggsave(share_svm, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/Share_SVM.pdf", height = 5, width = 10)
ggsave(share_lasso, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/Share_Lasso.pdf", height = 5, width = 10)
...
ggsave(p_boost_tuned, filename = "/Volumes/Transcend/Uni/doktorat/Proposal_Seminar/Paper 1/scripts/fold_and_oos_xgb_tuned.pdf", height = 6, width = 13)

...
abs <- read_excel("/Users/marcowyss/master-thesis-mw/Data/EEG_Daten_clean.xlsx", 
...
abs$Renewables<- factor(abs$Renewables)
abs$Renewables<- factor(abs$Solar)
abs$Renewables<- factor(abs$Water)
abs$Renewables<- factor(abs$Wind)
abs$Renewables<- factor(abs$Geo)
abs$Renewables<- factor(abs$Bio)
abs$Renewables<- factor(abs$Efficiency)
abs$Renewables<- factor(abs$Nuclear)
abs$Renewables<- factor(abs$`Environmental Protection`)
abs$Renewables<- factor(abs$`Security of Electricity Supply`)
abs$Renewables<- factor(abs$`Avoidance of Dependencies`)
abs$Renewables<- factor(abs$`Costs of Electricity`)
abs$Renewables<- factor(abs$`Cheaper RE Technology`)
abs$Renewables<- factor(abs$`Competitiveness of Industry`)
abs$Renewables<- factor(abs$`Value Chain`)
abs$Renewables<- factor(abs$Innovation)
abs$Renewables<- factor(abs$Employment)
abs$Renewables<- factor(abs$Export)
abs$Renewables<- factor(abs$`Energy Democracy`)
abs$Renewables<- factor(abs$`Climate Change Mitigation`)
abs$Renewables<- factor(abs$`Pollution Reduction`)
abs$Renewables<- factor(abs$`Mitigation of Nuclear Risk`)
abs$Renewables<- factor(abs$`Substitution of Limited Resources`)
abs$Renewables<- factor(abs$`Sector Coupling`)
abs$Renewables<- factor(abs$`Grid Extension`)
abs$Renewables<- factor(abs$`Energy Storage`)
abs$Renewables<- factor(abs$`Increase of Actor Variety`)
abs$Renewables<- factor(abs$`Costs for Consumers`)
abs$Renewables<- factor(abs$`Costs for Households`)
abs$Renewables<- factor(abs$`Costs for Industry`)
abs$Renewables<- factor(abs$`Employment in RE Sector`)
abs$Renewables<- factor(abs$`Employment not related to RE Sector`)
abs$Renewables<- factor(abs$`Employment in Solar Industry`)
...
forest_perf <- as.data.frame(evaluate_model_performance(res_forest, model_forest))
...
# ggsave(p_boost, filename = "/Users/marcowyss/master-thesis-mw/Graphics/Model Performance/Renewables/fold_and_oos_xgb.pdf", height = 6, width = 13)
# ggsave(p_svm, filename = "/Users/marcowyss/master-thesis-mw/Graphics/Model Performance/Renewables/fold_and_oos_svm.pdf", height = 6, width = 13)
ggsave(p_forest, filename = "/Users/marcowyss/master-thesis-mw/Graphics/Model Performance/Renewables/fold_and_oos_forest.pdf", height = 6, width = 13)
# ggsave(p_lasso, filename = "/Users/marcowyss/master-thesis-mw/Graphics/Model Performance/Renewables/fold_and_oos_lasso.pdf", height = 6, width = 13)
